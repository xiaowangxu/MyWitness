...
static func fit_block(puzzle_data : PuzzleData, start_area : Area, areas: Array, used_areas : Array, rule: AreaLinkFill) -> bool:
	...
	for i in range(0, rule.fill_rule.size(), 4):
		...
		if jump_edge == null: return false
...
static func solve_tetris(puzzle_data : PuzzleData, areas: Array, used_areas : Array, rules: Array) -> bool:
	...
	for area in areas:
		...
		var fitted : bool = AreaLinkFill.fit_block(puzzle_data, area, _areas, _used_areas, rule)
...
static func check_grouped_rule(puzzle_data : PuzzleData, lines_data : Array, grouped_rules : Array[PuzzleRule], isolated_area : Array) -> Array:
	...
	var ans = AreaLinkFill.solve_tetris(puzzle_data, _areas, [], _rules)
