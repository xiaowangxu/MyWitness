...
static func fit_block(puzzle_data : PuzzleData, start_area : Area, areas: Array, used_areas : Array, rule: AreaLinkFill, line_data : PackedInt32Array = [] as PackedInt32Array) -> bool:
	...
	for i in range(0, rule.fill_rule.size(), 4):
...
static func solve_hollow_tetris(puzzle_data : PuzzleData, areas: Array, used_areas : Array, rules: Array, hollow_rules: Array, line_data : PackedInt32Array = [] as PackedInt32Array) -> bool:
	if hollow_rules.size() == 0: 
#		print(areas)
		return AreaLinkFill.solve_tetris(puzzle_data, areas, [], rules, line_data)
	var rule : AreaLinkFill = hollow_rules.pop_front()
	for area in areas:
		var _areas : Array[Area] = areas.duplicate()
		var _used_areas : Array[Area] = used_areas.duplicate()
		var fitted : bool = AreaLinkFill.fit_block(puzzle_data, area, _areas, _used_areas, rule, line_data)
		if not fitted: continue
		var remain_result : bool = AreaLinkFill.solve_hollow_tetris(puzzle_data, _areas, _used_areas, rules, hollow_rules.duplicate())
		if remain_result: return true
	return false
